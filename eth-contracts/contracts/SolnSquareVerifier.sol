...
contract SolnSquareVerifier is ERC721MintableComplete{
    ...
    constructor(address verifierAddr) ERC721MintableComplete() public
    {
        squareContract = SquareVerifier(verifierAddr);
    }
...
//  - make sure you handle metadata as well as tokenSuplly
    ...
    function mintToken(address _to, uint256 _tokenId, uint[2] memory a, uint[2][2] memory b, uint[2] memory c, uint[2] memory input) public 
    {
        ...
        //  - make sure the solution is unique (has not been used before)
        require(uniqueSolutions[key].to == address(0), "Solution is used already");
        //  verify solution
        require(squareContract.verifyTx(a, b, c, input), "Solution isnt correct");
